...
os.environ["WANDB_DIR"] = "/scratch/dmuir/wandb/"
os.environ["WANDB_ARTIFACT_DIR"] = "/scratch/dmuir/wandb/artifacts/"
os.environ["WANDB_CACHE_DIR"] = "/scratch/dmuir/wandb/cache/"
os.environ["WANDB_DATA_DIR"] = "/scratch/dmuir/wandb/data/"
...
def main():
    ...
    parser.add_argument("--log_dir", default="/scratch/dmuir/wandb/" , type=Path, help="Directory to save logs")
    ...
    layer_list = list(range(config.min_layer, config.max_layer + 1))
    ...
    for layer in layer_list:
        ...
        for i in range(5):
            ...
            data_module = hydra.utils.instantiate(data_config, split_num=i, layer=layer)
            ...
            logger = WandbLogger(project=wandb_config.project,
                                name=wandb_config.run_name + f"_layer_{layer}_fold_{i}",
                                log_model=True,
            ...
            cleaned_config["data"]["layer"] = layer
